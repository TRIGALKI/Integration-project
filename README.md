...
```
Если мы хотим запускать скаченный докер образ то нужно также выполнить шаги 1-2 для получения моделей.
И запустить скаченный докер образ см раздел Docker.
В таком виде контейнер обеспечивает работу непосредственно приложения , в перспективе можно добавать сами файлы модели в докер образ и тогда тогда будет достаточно только скачать обрз (пулл) и запустить (run) но эта мысль пришла уже после сдачи.
