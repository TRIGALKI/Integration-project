...
│                         `1.0-jqp-initial-data-exploration. Тут ноутбук для обучения моделей!`.
│
...
│
│
── AB_test.md   <- Описание АБ теста
│
── API_doc.md   <- Описание API приложения
│
── ARCHITECTURE.md   <- Описание архмитектуры приложения
│
── MLOPS.md   <- Описание MLOPS инструментов
│
── Metrics.md   <- Описание метрик которые бизнес сможет использовать
│
── Dockrfile   <- Инструкция по сборке докер image
│
── ONYX_ML&uWSGI + NGINX.md Объяснение, для чего используются uWSGI + NGINX в production-среде.и перевод модели в оникс
...
### 2. Обучить модели (Опционально , на гите есть модели версии 1 и 2 , если не нужно добавлять еще или преобучать пропускаем этот шаг)
...
```
Если мы хотим запускать скаченный докер образ нужно также выполнить шаги 1-2 для получения моделей.
И запустить скаченный докер образ : см раздел Docker.
В таком виде контейнер обеспечивает работу непосредственно приложения , в перспективе можно добавать сами файлы модели в докер образ и тогда тогда будет достаточно только скачать обрз (pull) и запустить (run) но эта мысль пришла уже после сдачи.
